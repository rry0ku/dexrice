...
hl.bind(
	KEY.VIDEO,
	hl.dsp.exec_cmd("mpv --player-operation-mode=pseudo-gui --force-window=immediate"),
...
hl.bind(
	KEY.WORKSPACE_NEXT_MONITOR,
	hl.dsp.exec_cmd(scrPath .. "/workspace.sh"),
	{ description = "Next workspace on monitor" }
)
hl.bind(
	KEY.WORKSPACE_PREV_MONITOR,
...
hl.bind(KEY.RESIZE_RIGHT, function()
	hl.exec_cmd("hyprctl dispatch resizeactive 30 0")
end, { description = "Resize window right" })
hl.bind(KEY.RESIZE_LEFT, function()
	hl.exec_cmd("hyprctl dispatch resizeactive -30 0")
end, { description = "Resize window left" })
hl.bind(KEY.RESIZE_UP, function()
	hl.exec_cmd("hyprctl dispatch resizeactive 0 -30")
end, { description = "Resize window up" })
hl.bind(KEY.RESIZE_DOWN, function()
	hl.exec_cmd("hyprctl dispatch resizeactive 0 30")
end, { description = "Resize window down" })

hl.bind(KEY.MOVE_SWAP_LEFT, function()
	hl.exec_cmd(
		"bash -c 'grep -q true <<< $(hyprctl activewindow -j | jq -r .floating) && hyprctl dispatch moveactive -30 0 || hyprctl dispatch movewindow l'"
	)
end, { description = "Move or swap window left" })
hl.bind(KEY.MOVE_SWAP_RIGHT, function()
	hl.exec_cmd(
		"bash -c 'grep -q true <<< $(hyprctl activewindow -j | jq -r .floating) && hyprctl dispatch moveactive 30 0 || hyprctl dispatch movewindow r'"
	)
end, { description = "Move or swap window right" })
hl.bind(KEY.MOVE_SWAP_UP, function()
	hl.exec_cmd(
		"bash -c 'grep -q true <<< $(hyprctl activewindow -j | jq -r .floating) && hyprctl dispatch moveactive 0 -30 || hyprctl dispatch movewindow u'"
	)
end, { description = "Move or swap window up" })
hl.bind(KEY.MOVE_SWAP_DOWN, function()
	hl.exec_cmd(
		"bash -c 'grep -q true <<< $(hyprctl activewindow -j | jq -r .floating) && hyprctl dispatch moveactive 0 30 || hyprctl dispatch movewindow d'"
	)
end, { description = "Move or swap window down" })

-- 4. Screenshots
hl.bind(
	KEY.SHOT_WINDOW,
	hl.dsp.exec_cmd("HYPRSHOT_DIR=~/Pictures/Screenshots hyprshot -m window"),
	{ description = "Screenshot window" }
)
hl.bind(
	"ALT + PRINT",
	hl.dsp.exec_cmd("HYPRSHOT_DIR=~/Pictures/Screenshots hyprshot -m output"),
	{ description = "Screenshot monitor" }
)
hl.bind(
	"SHIFT + PRINT",
	hl.dsp.exec_cmd("HYPRSHOT_DIR=~/Pictures/Screenshots hyprshot -m region"),
...
hl.bind(
	...
	{ description = "Annotate screenshot" }
)
